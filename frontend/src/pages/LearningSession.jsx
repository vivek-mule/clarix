...
function LiveReasoningCard({
  ...
  const steps = [
    ...
    },
    {
      id: "retrieval",
      icon: FiDatabase,
      label: "Retrieving relevant content from knowledge base...",
      color: "#06b6d4",
    },
    {
      id: "generation",
      icon: FiBookOpen,
      label: "Generating personalized response...",
      color: "#a855f7",
    },
  ];
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background:
          "linear-gradient(135deg, rgba(99,102,241,0.04), rgba(168,85,247,0.03))",
...
export default function LearningSession() {
  ...
  const awaitingQuiz = Boolean(
    sessionMeta?.quiz_results?.awaiting_answers && quizQuestions?.length,
  );
  ...
  const loadSidebar = useCallback(() => {
    getAgentSessions()
      .then((rows) => setAllSessions(Array.isArray(rows) ? rows : []))
      .catch(() => {});
  }, []);

  useEffect(() => {
    loadSidebar();
  }, [sessionId, loadSidebar]);

  const reloadMetadata = useCallback(
    (reloadSidebar = false) => {
      if (!sessionId) return;
      getAgentSession({ session_id: sessionId })
        .then((res) => {
          setSessionMeta((prev) => ({
            ...prev,
            ...
            mastery_score:
              typeof res.mastery_score === "number" ? res.mastery_score : -1,
            retrieved_chunks: res.retrieved_chunks || [],
          }));
        })
        .catch(console.error);

      if (reloadSidebar) loadSidebar();
    },
    [sessionId, loadSidebar],
  );

  const hasFetchedStartRef = useRef(false);

  // Sync real-time reasoning window context exactly when the agent text first arrives
  useEffect(() => {
    if (isStreaming && streamedText.length > 0 && !hasFetchedStartRef.current) {
      hasFetchedStartRef.current = true;
      reloadMetadata(false);
    }
    if (!isStreaming) {
      hasFetchedStartRef.current = false;
    }
  }, [isStreaming, streamedText, reloadMetadata]);
  ...
  // Commit streamed text & safely reload metadata after stream finishes
  useEffect(() => {
    if (isStreaming) {
    ...
    if (wasStreamingRef.current) {
      ...
      resetStream();
      reloadMetadata(true); // reload metadata and sidebars when agent completes turn
    }
  }, [isStreaming, streamedText, resetStream, reloadMetadata]);

  const canStart = useMemo(() => !sessionId, [sessionId]);
  const currentModule = sessionMeta?.current_module || {};
  const masteryScore =
    typeof sessionMeta?.mastery_score === "number" &&
    sessionMeta.mastery_score >= 0
      ? sessionMeta.mastery_score
      : null;
  ...
  const reasoningSteps = useMemo(() => {
    ...
    return [
      ...
      {
        icon: FiTarget,
        label: "Evaluating mastery",
        color: "#10b981",
        status: masteryScore !== null ? "done" : "pending",
      },
    ];
  }, [
    currentAgent,
  ...
  const startSession = useCallback(async () => {
    ...
    const firstMessage =
      input.trim() ||
      "Help me learn this topic from basics with practical examples.";
    ...
    try {
      ...
      setActionError(
        e?.response?.data?.detail || e?.message || "Could not start session",
      );
      setIsThinking(false);
      setIsBusy(false);
    }
  ...
  const sendMessage = useCallback(async () => {
    ...
    setIsThinking(true);
    try {
      const res = await agentSubmitAnswer({
  ...
  const readyForQuiz = useCallback(async () => {
    ...
    setMessages((prev) => [
      ...prev,
      { role: "user", content: "I'm ready for a quiz now." },
    ]);
    try {
      const res = await agentSubmitAnswer({
        session_id: sessionId,
        answer: "I'm ready for a quiz now.",
      });
      setSessionMeta(res);
      setIsThinking(false);
      setIsBusy(false);
      ...
      setActionError(
        e?.response?.data?.detail || e?.message || "Could not start quiz",
      );
      setIsThinking(false);
      setIsBusy(false);
    }
  ...
  const submitQuiz = useCallback(
    async (answersArray) => {
      ...
      try {
        ...
        setActionError(
          e?.response?.data?.detail || e?.message || "Quiz submission failed",
        );
        setIsThinking(false);
      } finally {
            ...
            >
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "0.4rem" }}>
                {allSessions.length === 0 && (
                  <div
                    style={{ padding: "1.5rem 0.75rem", textAlign: "center" }}
                  >
                    <FiMessageSquare
                      size={20}
                      style={{
                ...
                {allSessions.map((s) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <FiClock size={9} />{" "}
                        {fmtDate(s.last_message_at || s.updated_at)}
                      </div>
                    </div>
                  );
                })}
        ...
        >
          ...
          >
              ...
              <div>
                ...
                {sessionId && (
                  ...
                  >
                    ...
                    {masteryScore !== null && (
                      <>
                        <span style={{ opacity: 0.4 }}>·</span> Mastery{" "}
            ...
            >
              ...
              <button
                ...
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = rightPanelOpen
                    ? "rgba(255,255,255,0.04)"
                    : "none";
                }}
              >
          ...
          >
            ...
            {(streamedText || isStreaming) && (
              <Bubble role="assistant" isStreaming={isStreaming}>
                {streamedText || ""}
              </Bubble>
            )}
            ...
            {!messages.length && !streamedText && !isThinking && (
              <div style={{ flex: 1, display: "grid", placeItems: "center" }}>
                ...
                >
                  ...
                  >
                    Ask about any topic you want to learn. Clarix will analyze
                    your level, retrieve relevant materials, and teach you with
                    adaptive content.
                  </p>
                  <div
                    style={{
          ...
          >
            ...
            >
              Clarix uses AI to teach adaptively. Responses are grounded in your
              course materials.
            </div>
          </div>
        </div>
              ...
              >
                  ...
                  <div style={{ marginBottom: "1.25rem" }}>
                    ...
                    >
                      {[
                        {
                          label: "Module",
                          val: currentModule?.topic || "Awaiting",
                          icon: FiHash,
                        },
                        {
                          label: "Mastery",
                ...
                {!sessionId && (
                  <div style={{ padding: "2rem 0.75rem", textAlign: "center" }}>
                    ...
                    >
                      Start a session to see the agent reasoning pipeline and
